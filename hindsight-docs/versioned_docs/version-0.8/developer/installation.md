...
If you instead bind-mount a **host directory** (`-v $HOME/.hindsight-docker:/home/username/.pg0`), that directory must be writable by UID 1000, or the embedded database fails to start with `Permission denied`. Either `chown` the directory to UID 1000, or run the container as your host user: `--user $(id -u):$(id -g) -e HOME=/home/username` (after `chown`-ing the directory to your own UID).
:::
