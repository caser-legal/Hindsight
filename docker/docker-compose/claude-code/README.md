...
docker exec --user 0:0 hindsight-claude-code chown $(id -u):$(id -g) /home/username/.claude
docker exec --user 0:0 hindsight-claude-code chmod 755 /home/username/.claude

# Symlink the host claude binary into PATH
docker exec --user 0:0 hindsight-claude-code \
  ln -sf /home/username/.local/share/claude/versions/2.1.128 /usr/local/bin/claude
```
...
- **`--user` / `user:`** — the `user: ${HOST_UID}:${HOST_GID}` pattern
  requires `chmod 755 /home/username`, which is built into the image since
...
```bash
docker exec --user 0:0 hindsight-claude-code \
  ln -sf /home/username/.local/share/claude/versions/2.2.0 /usr/local/bin/claude
```
