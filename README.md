...
This tree is an independent snapshot of Hindsight (see `UPSTREAM_SNAPSHOT.md`) plus optional local scripts. You can run the upstream Docker image, or the standalone macOS setup in this repo.
...
docker run -it --pull always --name hindsight --restart unless-stopped -p 8888:8888 -p 9999:9999 \
  ...
  -v hindsight-data:/home/username/.pg0 \
...
Requirements: Python 3.11–3.13, Node.js, and PostgreSQL 14+ with a vector extension (the local scripts target Homebrew PostgreSQL 17 and pgvector).

```bash
cp .env.9router.example .env
# Set HINDSIGHT_API_LLM_API_KEY and HINDSIGHT_API_DATABASE_URL. Do not commit .env.
./scripts/local/setup-standalone.sh
./scripts/local/start-standalone.sh
```

Point `HINDSIGHT_API_LLM_BASE_URL` at any OpenAI-compatible endpoint, including a local router. Runtime files stay under `~/.local/share/hindsight`. Daily backups are installed with `./scripts/local/install-backup-launch-agent.sh`, which fills in your home directory when it installs the launch agent.

Details: `LOCAL_9ROUTER_SETUP.md`.
