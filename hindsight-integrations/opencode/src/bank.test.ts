...
describe("deriveBankId", () => {
  ...
  it("returns default bank name in static mode", () => {
    expect(deriveBankId(makeConfig(), "/home/username/project")).toBe("opencode");
  });

  it("returns configured bankId in static mode", () => {
    const config = makeConfig({ bankId: "my-bank" });
    expect(deriveBankId(config, "/home/username/project")).toBe("my-bank");
  });

  it("adds prefix in static mode", () => {
    const config = makeConfig({ bankIdPrefix: "dev", bankId: "my-bank" });
    expect(deriveBankId(config, "/home/username/project")).toBe("dev-my-bank");
  });
  ...
  it("composes from granularity fields in dynamic mode", () => {
    ...
    expect(deriveBankId(config, "/home/username/my-project")).toBe("opencode::my-project");
  });
  ...
  it("uses default granularity when not specified", () => {
    ...
    expect(deriveBankId(config, "/home/username/proj")).toBe("opencode::proj");
  });
  ...
  it("preserves raw special characters", () => {
    ...
    expect(deriveBankId(config, "/home/username/my project")).toBe("my project");
  });
  ...
  it("preserves raw UTF-8 characters", () => {
    ...
    expect(deriveBankId(config, "/home/username/мой проект")).toBe("мой проект");
  });
  ...
  it("uses channel/user from env vars", () => {
    ...
    expect(deriveBankId(config, "/home/username/proj")).toBe("opencode::slack-general::user123");
  });
  ...
  it("uses defaults for missing env vars", () => {
    ...
    expect(deriveBankId(config, "/home/username/proj")).toBe("default::anonymous");
  });
  ...
  it("adds prefix in dynamic mode", () => {
    ...
    expect(deriveBankId(config, "/home/username/proj")).toBe("dev-opencode");
  });
  ...
  describe("project field stays directory-only (backwards compatibility)", () => {
    it("uses raw directory basename for `project` even inside a git repo", () => {
      mockExec.mockReturnValueOnce("/home/username/myproj/.git\n" as never);
      const config = makeConfig({
        dynamicBankId: true,
        dynamicBankGranularity: ["agent", "project"],
      });
    ...
    it("does not invoke git when only `project` is in the granularity", () => {
      ...
      deriveBankId(config, "/home/username/myproj");
  ...
  describe("gitProject field (git-aware)", () => {
    it("uses main worktree basename when running inside a regular clone", () => {
      ...
      expect(deriveBankId(config, "/home/username/myproj")).toBe("opencode::myproj");
    });
    ...
    it("returns the same bank id from a linked worktree of the same repo", () => {
      ...
      mockExec
        .mockReturnValueOnce("/home/username/myproj/.git\n" as never)
        .mockReturnValueOnce("/home/username/myproj/.git\n" as never);
      const config = makeConfig({
        dynamicBankId: true,
        dynamicBankGranularity: ["agent", "gitProject"],
      });
      const main = deriveBankId(config, "/home/username/myproj");
    ...
    it("does not invoke git in static mode", () => {
      ...
      expect(deriveBankId(config, "/home/username/myproj")).toBe("fixed");
      expect(mockExec).not.toHaveBeenCalled();
    });
    ...
    it("does not invoke git when gitProject is not in the granularity", () => {
      ...
      expect(deriveBankId(config, "/home/username/myproj")).toBe("opencode::default");
      expect(mockExec).not.toHaveBeenCalled();
    });
