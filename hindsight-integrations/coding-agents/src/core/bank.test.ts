...
describe("deriveBankId", () => {
  ...
  it("defaults to coding-agent::<directory basename> outside git", () => {
    expect(deriveBankId({}, "/home/username/scratch")).toBe("coding-agent::scratch");
  });

  it("resolves the MAIN worktree root inside git (worktrees share one bank)", () => {
    mockExec.mockReturnValue("/home/username/dev/myrepo/.git\n");
    expect(deriveBankId({}, "/home/username/dev/myrepo-feature-wt")).toBe("coding-agent::myrepo");
  });
  ...
  it("resolveWorktrees=false skips git and uses the directory basename", () => {
    mockExec.mockReturnValue("/home/username/dev/myrepo/.git\n");
    expect(deriveBankId({ resolveWorktrees: false }, "/home/username/dev/myrepo-wt")).toBe(
  ...
  it("the default bank is harness-neutral — same id regardless of the harness arg", () => {
    expect(deriveBankId({}, "/home/username/scratch", "claude-code")).toBe("coding-agent::scratch");
    expect(deriveBankId({}, "/home/username/scratch", "codex")).toBe("coding-agent::scratch");
  });
  ...
  it("dynamicBankId=true overrides an explicit bankId", () => {
    expect(deriveBankId({ bankId: "pinned", dynamicBankId: true }, "/home/username/proj")).toBe(
      "coding-agent::proj"
    );
  });

  it("dynamicBankId=false without bankId falls back to the default name", () => {
    expect(deriveBankId({ dynamicBankId: false }, "/home/username/proj")).toBe("coding");
  });

  describe("bankIdTemplate", () => {
    it("supports literal text around placeholders", () => {
      expect(deriveBankId({ bankIdTemplate: "hindsight-{gitProject}" }, "/home/username/proj")).toBe(
        "hindsight-proj"
      );
    });
    ...
    it("fills {harness} from the caller", () => {
      expect(
        deriveBankId({ bankIdTemplate: "{harness}-{gitProject}" }, "/home/username/proj", "claude-code")
      ).toBe("claude-code-proj");
    });

    it("{project} is the plain directory basename even inside git", () => {
      mockExec.mockReturnValue("/home/username/dev/myrepo/.git\n");
      expect(deriveBankId({ bankIdTemplate: "{project}" }, "/home/username/dev/myrepo-wt")).toBe(
        "myrepo-wt"
      );
    });
    ...
    it("warns on unknown placeholders and substitutes 'unknown'", () => {
      ...
      expect(deriveBankId({ bankIdTemplate: "x-{gitProjectId}" }, "/home/username/proj")).toBe(
        "x-unknown"
      );
  ...
  describe("mapPathToBank", () => {
    const WT = "/home/username/dev/myrepo";
