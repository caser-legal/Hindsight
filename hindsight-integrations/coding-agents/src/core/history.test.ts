...
describe("local history import", () => {
  it("reads Claude sessions from the project directory for THIS repo only", () => {
    const h = newHome();
    const repo = "/Users/username/dev/myrepo";
    ...
    const other = claudeProjectDir("/Users/username/dev/otherrepo", h);
    mkdirSync(other, { recursive: true });
    writeFileSync(
      join(other, "s2.jsonl"),
      `${claudeLine("user", "unrelated", "/Users/username/dev/otherrepo")}\n`
    );
...
describe("attribution must be proven, never guessed", () => {
  ...
  it("does not pull a SIBLING repo's sessions in via the ambiguous name encoding", () => {
    const h = newHome();
    // "/Users/username/dev/repo-sub" and "/Users/username/dev/repo/sub" both encode to the same prefix shape,
    // so only the recorded cwd can tell them apart.
    const sibling = claudeProjectDir("/Users/username/dev/repo-sub", h);
    mkdirSync(sibling, { recursive: true });
    writeFileSync(
      join(sibling, "s.jsonl"),
      `${claudeLine("user", "sibling repo work", "/Users/username/dev/repo-sub")}\n`
    );

    const r = importLocalHistory("claude-code", "/Users/username/dev/repo", h);
    expect(JSON.stringify(r.sessions)).not.toContain("sibling repo work");
  });

  it("includes a session run in a SUBDIRECTORY of the repo", () => {
    const h = newHome();
    const repo = "/Users/username/dev/repo";
    const sub = claudeProjectDir("/Users/username/dev/repo/packages/api", h);
    mkdirSync(sub, { recursive: true });
    writeFileSync(
      join(sub, "s.jsonl"),
      `${claudeLine("user", "work in a subpackage", "/Users/username/dev/repo/packages/api")}\n`
    );
