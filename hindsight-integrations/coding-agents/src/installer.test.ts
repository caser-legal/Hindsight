...
describe("grok-build installer", () => {
  ...
  const hooksPath = (ctx: InstallCtx) =>
    join(ctx.home, ".grok", "hooks", "hindsight-coding-agents.json");
  ...
  it("replaces an unmarked legacy Hindsight MCP table", () => {
    const ctx = makeCtx();
    ...
    expect(config).not.toContain("/old/mcp-server.js");
    expect(config).toContain('[agent]\nname = "grok"');
  });

  it("installs global Grok lifecycle hooks and MCP without Claude configuration", () => {
    const ctx = makeCtx();
    ...
    expect(hooks.SessionStart[0].hooks[0].command).toContain(
      join(ctx.dist, "grok-sessionstart-hook.js")
    );
    expect(hooks.UserPromptSubmit[0].hooks[0].command).toContain(join(ctx.dist, "grok-hook.js"));
    expect(hooks.Stop[0].hooks[0].command).toContain(join(ctx.dist, "grok-stop-hook.js"));
