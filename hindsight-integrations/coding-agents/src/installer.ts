...
/** Grok may rewrite config.toml and discard our comments while preserving the MCP table. */
const GROK_MCP_RE = /\n?\[mcp_servers\.hindsight\]\n(?:^(?!\s*\[).*(?:\n|$))*/gm;
...
const grok: HarnessInstaller = {
  ...
  install(c) {
    const hooksPath = join(c.home, ".grok", "hooks", "hindsight-coding-agents.json");
    const hooks: Record<string, any> = {};
    ...
    c.log?.(`grok-build: global hooks installed at ${hooksPath}, MCP installed in ${path}`);
  },
  uninstall(c) {
    rmSync(join(c.home, ".grok", "hooks", "hindsight-coding-agents.json"), { force: true });
    const path = join(c.home, ".grok", "config.toml");
    if (existsSync(path)) {
    ...
    c.log?.("grok-build: global hooks + MCP + skill removed");
  },
};
