...
def _hook(session_id="sess-1", cwd="/home/username/myproject"):
...
class TestDeriveBankIdDynamic:
    def test_dynamic_agent_project(self):
        ...
        result = derive_bank_id(_hook(cwd="/home/username/hindsight"), cfg)
    ...
    def test_dynamic_preserves_raw_special_chars(self):
        ...
        result = derive_bank_id(_hook(cwd="/home/username/my project"), cfg)
        assert "my project" in result
        assert "%" not in result

    def test_dynamic_preserves_raw_utf8(self):
        cfg = _cfg(dynamicBankId=True, dynamicBankGranularity=["project"])
        result = derive_bank_id(_hook(cwd="/home/username/мой проект"), cfg)
    ...
    def test_dynamic_worktree_resolves_to_main_repo(self, mock_run):
        ...
        mock_result.stdout = "/home/username/myproject/.git\n"
        ...
        result = derive_bank_id(_hook(cwd="/home/username/myproject-wt1"), cfg)
...
class TestResolveProjectName:
    ...
    def test_regular_repo(self, mock_run):
        mock_run.return_value = self._mock_git("/home/username/myproject/.git\n")
        assert _resolve_project_name("/home/username/myproject", _cfg()) == "myproject"

    @patch("lib.bank.subprocess.run")
    def test_worktree_resolves_to_main_repo(self, mock_run):
        # Worktree at /home/username/myproject-wt1, main repo at /home/username/myproject
        mock_run.return_value = self._mock_git("/home/username/myproject/.git\n")
        assert _resolve_project_name("/home/username/myproject-wt1", _cfg()) == "myproject"

    @patch("lib.bank.subprocess.run")
    def test_worktree_different_location(self, mock_run):
        # Worktree at /tmp/worktrees/feature-x, main repo at /home/username/hindsight
        mock_run.return_value = self._mock_git("/home/username/hindsight/.git\n")
    ...
    def test_disabled_falls_back_to_basename(self, mock_run):
        ...
        assert _resolve_project_name("/home/username/myproject-wt1", cfg) == "myproject-wt1"
    ...
    def test_not_a_git_repo(self, mock_run):
        ...
        assert _resolve_project_name("/home/username/plaindir", _cfg()) == "plaindir"
...
class TestDirectoryBankMap:
    ...
    def test_exact_match(self):
        cfg = _cfg(directoryBankMap={"/home/username/myproject": "custom-bank"})
        result = derive_bank_id(_hook(cwd="/home/username/myproject"), cfg)
    ...
    def test_no_match_falls_through_to_static(self):
        cfg = _cfg(directoryBankMap={"/home/username/other": "other-bank"}, bankId="default-bank")
        result = derive_bank_id(_hook(cwd="/home/username/myproject"), cfg)
        assert result == "default-bank"

    def test_no_match_falls_through_to_dynamic(self):
        cfg = _cfg(
            directoryBankMap={"/home/username/other": "other-bank"},
            dynamicBankId=True,
            agentName="bot",
    ...
    def test_with_prefix(self):
        cfg = _cfg(
            directoryBankMap={"/home/username/myproject": "custom-bank"},
            bankIdPrefix="prod",
        )
        result = derive_bank_id(_hook(cwd="/home/username/myproject"), cfg)
    ...
    def test_overrides_dynamic_mode(self):
        cfg = _cfg(
            directoryBankMap={"/home/username/myproject": "explicit-bank"},
            dynamicBankId=True,
            agentName="bot",
            dynamicBankGranularity=["agent", "project"],
        )
    ...
    def test_multiple_entries(self):
        cfg = _cfg(directoryBankMap={
            "/home/username/project-a": "bank-a",
            "/home/username/project-b": "bank-b",
        })
        assert derive_bank_id(_hook(cwd="/home/username/project-a"), cfg) == "bank-a"
        assert derive_bank_id(_hook(cwd="/home/username/project-b"), cfg) == "bank-b"
